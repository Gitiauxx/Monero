...
res_fig <- read.csv("../../results/changes_shape_passive_attack_size.csv")
res_fig <- data.table(res_fig)


tikz(file = "..\\figure_chainsize.tex", width = 2.75, height = 2.75)

plt1 <- ggplot(res_fig)
plt1 <- plt1 + geom_point(aes(x=size, y=map_unknown_distribution, color="blue"),  show.legend=F)
plt1 <- plt1 + geom_line(aes(x=size, y=map_unknown_distribution, color="blue") )            
plt1 <- plt1 + theme(panel.grid.major = element_line(colour = "gray"),
                     panel.background = element_blank(), axis.line = element_line(colour = "black"))

plt1 <- plt1 + scale_x_continuous(limits = c(10000, 100000))
plt1 <- plt1 + scale_y_continuous(limits = c(0, 0.2))
...
plt1 <- plt1 + labs(x="Number of transactions", y="Succes rate")
plt1
